...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { LLAMA_CLOUD_API_KEY: requireValue(readEnv('LLAMA_CLOUD_API_KEY') ?? client.apiKey, 'set LLAMA_CLOUD_API_KEY environment variable or provide apiKey client option'), LLAMA_CLOUD_BASE_URL: readEnv('LLAMA_CLOUD_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@llamaindex/llama-cloud';
