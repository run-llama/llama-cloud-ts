...
import * as FilesAPI from '../../v1/files/files';
...
export class Job extends APIResource {
  ...
  retrieve(jobID: string, options?: RequestOptions): APIPromise<ParsingAPI.ParsingJob> {
    return this._client.get(path`/api/parsing/job/${jobID}`, options);
  }
  ...
  generatePresignedURL(
    ...
    const { job_id } = params;
    return this._client.get(path`/api/parsing/job/${job_id}/read/${filename}`, options);
  }
  ...
  getDetails(jobID: string, options?: RequestOptions): APIPromise<unknown> {
    return this._client.get(path`/api/parsing/job/${jobID}/details`, options);
  }
  ...
  getParameters(jobID: string, options?: RequestOptions): APIPromise<unknown> {
    return this._client.get(path`/api/parsing/job/${jobID}/parameters`, options);
  }
}
